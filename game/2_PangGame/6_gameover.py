...
total_time = 100
start_ticks = pygame.time.get_ticks() #시작 시간정의
game_result = 'Game Over' #종료 메시지
...
while running:
    ...
    for ball_index, ball_val in enumerate(balls):
        ...
        for weapon_inx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                # 가장 작은 크기의 공이 아니면 쪼개짐
                if ball_img_idx < 3: # 0,1,2 의공이라면 

                    #현재 공 크기 값을 가져옴 
                    ball_widh = ball_rect[0]
                    ball_height = ball_rect[1]

                    #나눠진 공의 정보
                    small_ball_rect = ball_imgaes[ball_img_idx + 1].get_rect()
                    small_ball_rect_widh = small_ball_rect[0]
                    small_ball_rect_height = small_ball_rect[1]

                    # 왼쪽으로 튕기는 공 
                    balls.append({
                        'pos_x' : ball_pos_x + (ball_widh/2) - (small_ball_rect_widh/2) , # x좌표
                        'pos_y' : ball_pos_y + (ball_height/2) - (small_ball_rect_height/2), # y좌표
    ...
    timer = game_font.render("Time : {}".format(int(total_time-elapsed_time)),True,('white'))
...
# 게임오버 메시지
msg = game_font.render(game_result,True,('tomato')) #노란색
msg_rect = msg.get_rect(center = (int(screen_widh/2), int(screen_height/2)) )
screen.blit(msg,msg_rect)

#  화면에 반영이 되지 않기때문에 다시 업데이트!!
pygame.display.update()

# 메시지 띄우고 2초대기 >> 대기 하지않으면 메시지가 뜨자마자 게임이 종료하기 때문
